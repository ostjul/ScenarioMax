...
def convert_waymo_perception_scenario(waymo_scenario: Any, version: str) -> UnifiedScenario:
    ...
    scenario = UnifiedScenario(scenario_id=scenario_id, dataset_name="WOD", dataset_version=version)
    
    ###### In debug mode transfer all scenes to the mesh center
    import os
    ...
    # debug_mode = True
    if debug_mode:
        logger.info("🐛 DEBUG MODE: Transforming and centering wrt. mesh.")
        scene_tfrecor = waymo_scenario['scenario_id'].split('|')[1]

        if "validation" in scene_tfrecor:
        ...
        center_file = glob.glob(os.path.join(base_pth, scene_tfrecor.split('/')[-1][:14], 'pcd', 'center*'))[0]
        scene_pose = np.loadtxt(center_file).reshape(4,4).astype(np.float32)
    else:
        scene_pose = None

    ######
    scene_pose = None # TODO: Remove after debugging
    # Convert data
    dynamic_agents, ego_id = extract_dynamic_agents(waymo_scenario, scene_pose)
    static_map_elements = extract_static_map_elements(waymo_scenario, scene_pose)
...
def extract_dynamic_agents(waymo_scenario: Any, scene_pose: np.ndarray | None) -> dict[str, dict]:
    ...
    for track in waymo_scenario.tracks:
        ...
        unified_type = waymo_type # WOMD: unified_type = waymo_types.get_agent_type(waymo_type)
        ...
        if scene_pose is not None:
            pos_h = np.hstack((position, np.ones((position.shape[0], 1), dtype=position.dtype)))
            position = (scene_pose @ pos_h.T).T[:, :3]
            # Rotate velocity
            rot = scene_pose[:3, :3]
            velocity = (rot[:2, :2] @ velocity.T).T
            # Rotate heading
            heading_xy = np.stack([np.cos(heading), np.sin(heading)], axis=-1)
            heading_xy = (rot[:2, :2] @ heading_xy.T).T
            heading = np.arctan2(heading_xy[:, 1], heading_xy[:, 0])
...
def extract_static_map_elements(waymo_scenario: Any, scene_pose: np.ndarray | None) -> dict[str, dict]:
    ...
    for map_feature in waymo_scenario.map_features:
        ...
        if scene_pose is not None:
            if "polyline" in static_map_elements[element_id]:
                static_map_elements[element_id]["polyline"] = (scene_pose @ np.concatenate([static_map_elements[element_id]["polyline"], np.ones_like(static_map_elements[element_id]["polyline"][:, :1])], axis=-1).T).T[:, :3]
            elif "polygon" in static_map_elements[element_id]:
                static_map_elements[element_id]["polygon"] = (scene_pose @ np.concatenate([static_map_elements[element_id]["polygon"], np.ones_like(static_map_elements[element_id]["polygon"][:, :1])], axis=-1).T).T[:, :3]
            elif "position" in static_map_elements[element_id]:
                pos = static_map_elements[element_id]["position"]
                pos_h = np.hstack((pos, [1]))
                static_map_elements[element_id]["position"] = (scene_pose @ pos_h)[:3]
